...
class FinancialAccount(db.Model):
    ...
    # Balance
    total_owed = db.Column(db.Numeric(10, 2), default=0.00)  # Total adeudado
    total_paid = db.Column(db.Numeric(10, 2), default=0.00)  # Total pagado
    ...
    transactions = db.relationship('FinancialTransaction', backref='account', lazy='dynamic',
                                  cascade='all, delete-orphan', order_by='FinancialTransaction.created_at.desc()')
    
    @property
    def balance(self):
        """Balance actual (negativo = deuda, positivo = a favor)"""
        return float(self.total_paid) - float(self.total_owed)
